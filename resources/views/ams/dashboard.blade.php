...
<body class="min-h-screen bg-gray-50 font-sans antialiased">
        ...
        <div class="mx-auto max-w-7xl px-4 py-4 flex items-center gap-4">
            <div class="flex items-center gap-3">
                ...
                <form action="#" method="get" class="hidden md:flex items-stretch gap-2 ms-8">
                    <label for="header-course-search" class="sr-only">Search courses</label>
                    <input id="header-course-search" type="text" placeholder="  Search courses " class="w-72 rounded-md border-gray-300 focus:border-sublue-600 focus:ring-sublue-600" />
            ...
            <nav class="ms-auto hidden md:flex items-center gap-6 text-sm">
                ...
                @if (session()->has('member'))
                    <a href="{{ route('logout') }}" class="inline-flex items-center rounded-md px-3 py-1.5 text-white hover:opacity-90" style="background-color:#0a3a8c">Logout</a>
                @else
    ...
    <div class="mx-auto max-w-7xl px-4 py-6 grid grid-cols-12 gap-6">
        ...
        <!-- Temporary Pass quick action (centered) -->
        <div class="col-span-12">
            <div class="max-w-4xl mx-auto">
                <div class="bg-white rounded-xl border shadow-sm p-6 flex items-center justify-between">
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900">Temporary Pass</h2>
                        <p class="text-sm text-gray-600">Apply for a temporary pass as a university member.</p>
                    </div>
                    <a href="{{ route('tpas.members.apply') }}" class="inline-flex items-center rounded-md px-4 py-2 text-white hover:opacity-90" style="background-color:#0a3a8c">Register Now</a>
                </div>
            </div>
        </div>
        ...
        <main class="col-span-12 md:col-span-9 lg:col-span-9 space-y-6">
            ...
            <div class="max-w-6xl mx-auto">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    @php
                        $courses = [
                            ['title' => 'Business Analytics', 'img' => 'https://placehold.co/600x360?text=Business+Analytics'],
                            ['title' => 'Data Structures', 'img' => 'https://placehold.co/600x360?text=Data+Structures'],
                            ['title' => 'Database Systems', 'img' => 'https://placehold.co/600x360?text=Database+Systems'],
                            ['title' => 'Web Engineering', 'img' => 'https://placehold.co/600x360?text=Web+Engineering'],
                            ['title' => 'AI Foundations', 'img' => 'https://placehold.co/600x360?text=AI+Foundations'],
                            ['title' => 'Cyber Security', 'img' => 'https://placehold.co/600x360?text=Cyber+Security'],
                        ];
                    @endphp
                    @foreach ($courses as $course)
                        <article class="bg-white rounded-xl border shadow-sm overflow-hidden">
                            <img src="{{ $course['img'] }}" alt="{{ $course['title'] }}" class="w-full h-36 object-cover" />
                            <div class="p-4">
                                <h3 class="text-sm font-semibold text-gray-900">{{ $course['title'] }}</h3>
                                <button class="mt-3 inline-flex items-center text-sm rounded-md px-3 py-1.5 text-white" style="background-color:#0a3a8c">View</button>
                            </div>
                        </article>
                    @endforeach
                </div>
    ...
    <footer class="mt-10 text-white" style="background-color:#0a3a8c">
        <div class="mx-auto max-w-7xl px-4 py-10 grid grid-cols-1 md:grid-cols-3 gap-8">
            <div>
                <div class="flex items-center gap-3">
                    <img src="{{ asset('images/strathmore_logo.png') }}" alt="University Logo" class="h-16 w-auto object-contain" />
                </div>
