...
Secure login for students and staff using their existing university credentials (simulated via an external AMS database).

A public-facing portal for visitors to request temporary passes.

A centralized hub for administrators to review, approve, or reject pass applications.
...
Conditional Approval: Smart logic that instantly approves standard requests while flagging restricted cases (e.g., frequent lost IDs) for manual review.
...
Public: To view the main university landing page, simply go to / (Home).
## Prerequisites
